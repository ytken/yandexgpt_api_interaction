...
class YandexGPTChat:
    ...
    def save_to_file(self, filepath: str = CHAT_HISTORY_FILE):
        """
        ...
        try:
            ...
            print(f"⚠️ Ошибка при сохранении: {str(e)}")

    def load_from_file(self, filepath: str = CHAT_HISTORY_FILE) -> bool:
        """
        ...
        try:
            ...
            self.current_summary = data.get("current_summary", None)
            self.total_tokens = data.get("total_tokens", 0)
            
            print(f"✅ История загружена из {filepath}")
            print(f"📝 Загружено сообщений: {len(self.messages)}")
            print(f"📈 Токенов за предыдущую сессию: {self.total_tokens}\n")
            return True
        except Exception as e:
            print(f"⚠️ Ошибка при загрузке: {str(e)}")
            return False
    ...
    def _apply_summarization(self):
        ...
        if summary:
            ...
            # Сохраняем в файл после суммаризации
            self.save_to_file()
...
def interactive_chat():
    ...
    try:
        ...
        # Проверяем наличие сохраненной истории
        if os.path.exists(CHAT_HISTORY_FILE):
            while True:
                response = input("📂 Найдена сохраненная история чата. Загрузить её? (да/нет): ").strip().lower()

...
- get_pull_request_diff: получить diff конкретного PR
- get_pull_request_files: получить список файлов и сводку изменений в PR
...
- Для обзора изменений в PR → get_pull_request_diff и get_pull_request_files
...
Теперь дай финальный ответ пользователю, объединив информацию из документации и GitHub:"""
    ...
    async def review_pull_request(
        ...
        """
        Проводит ревью PR по его номеру, используя RAG и данные из GitHub.
        """
        # Запускаем MCP если не запущен
        if not self.mcp_started:
            await self.start_mcp()

        question = f"Сделай подробное ревью pull request #{pr_number}."
        self.add_to_history('user', question)

        # Контекст из документации/кода
        print("📚 Поиск контекста для ревью PR в документации...")
        context_chunks = self.rag_system.search_relevant_chunks(
            f"Ревью pull request #{pr_number}. Стандарты и архитектура проекта.",
            top_k=top_k
        )
        ...
        # Получаем данные по PR из GitHub через MCP
        print(f"🐙 Получение файлов PR #{pr_number}...")
        files_text = await self.mcp_client.call_tool(
            "get_pull_request_files",
            {"number": pr_number, "repo": self.github_repo}
        )

        print(f"🐙 Получение diff PR #{pr_number}...")
        diff_text = await self.mcp_client.call_tool(
...
ДОКУМЕНТАЦИЯ ПРОЕКТА (RAG контекст):
{doc_context}

ДАННЫЕ ИЗ GITHUB ПО PR:

ФАЙЛЫ И СВОДКА ИЗМЕНЕНИЙ:
{files_text}
...
ЗАДАЧА:
Ты выступаешь как строгий, но конструктивный ревьюер кода.
Сделай детальное ревью Pull Request #{pr_number} с учётом контекста проекта из документации.

Требуется:
1. Кратко описать, что делает этот PR и какие части системы затрагивает.
2. Перечислить основные достоинства изменений.
3. Найти возможные проблемы по категориям: архитектура, читаемость, надёжность, безопасность, производительность, тестируемость.
4. Для каждого замечания приводи как можно более точную привязку к файлам (и строкам, если это понятно из diff).
5. Дать конкретные рекомендации, как улучшить код, в виде списка.

Форматируй ответ в виде структурированного ревью с заголовками и списками.

ОТВЕТ:"""

        print("💬 Генерация ревью PR...\n")
        llm_model = self.sdk.models.completions(model)
        llm_model = llm_model.configure(temperature=temperature, max_tokens=2500)
        result = llm_model.run(prompt)
        answer = result.alternatives[0].text
        ...
        return {
            ...
            'context_chunks': context_chunks,
            'github_results': [
                {'tool': 'get_pull_request_files', 'result': files_text},
                {'tool': 'get_pull_request_diff', 'result': diff_text},
            ],
            'history_length': len(self.conversation_history),
            'used_github': True
        }
...
Команды:
  ...
  /review_pr <номер> - сделать ревью указанного PR
...
async def run_chat(index_path: str, github_repo: str):
    ...
    try:
        ...
        while True:
            ...
            # GitHub команды
            ...
            elif user_input.lower().startswith('/review_pr'):
                parts = user_input.split(maxsplit=1)
                if len(parts) < 2:
                    print("⚠️ Укажи номер PR, например: /review_pr 42\n")
                    continue
                try:
                    pr_number = int(parts[1])
                except ValueError:
                    print("⚠️ Номер PR должен быть целым числом, например: /review_pr 42\n")
                    continue

                print()
                result = await bot.review_pull_request(pr_number)

                print("\n" + "─"*80)
                print(f"🤖 Ревью PR #{pr_number}:")
                print("─"*80)
                print(result['answer'])
                ...
                print("\n" + "─"*80)
                print(f"📊 Статистика: 🐙 GitHub | История: {result['history_length']} сообщений")
                print("─"*80 + "\n")
                continue
